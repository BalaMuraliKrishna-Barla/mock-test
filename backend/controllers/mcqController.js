...
const sanitizeText = (text) => {
  if (!text) return "";
  // This regex replaces invalid Unicode characters, including lone surrogates,
...
const generateMCQFromFile = async (req, res) => {
  ...
  try {
    ...
    if (!sanitizedExtractedText || sanitizedExtractedText.trim().length < 100) {
      return res.status(422).json({
        message:
          "Could not extract sufficient valid text from the document. Please ensure it has at least 100 characters.",
      });
    } /* Sanitization Ends */
